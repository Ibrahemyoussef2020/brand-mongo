...
export default function CashiersStaffPage() {
  const { translate } = useLang();
  
  // Data State
  const [data, setData] = useState<CashiersStaffItem[]>([
    { id: '1', name: 'Sample Cashiers & Staff A', status: 'Active', date: 'Oct 24, 2026' },
    { id: '2', name: 'Sample Cashiers & Staff B', status: 'Pending', date: 'Oct 25, 2026' },
    { id: '3', name: 'Sample Cashiers & Staff C', status: 'Inactive', date: 'Oct 26, 2026' },
  ]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<CashiersStaffItem | null>(null);
  ...
  const handleEditClick = (record: CashiersStaffItem) => {
    setSelectedRecord(record);
    setFormName(record.name);
    setIsEditModalOpen(true);
  };

  const handleViewClick = (record: CashiersStaffItem) => {
    setSelectedRecord(record);
    setIsViewModalOpen(true);
  };

  const handleDeleteClick = (record: CashiersStaffItem) => {
    setSelectedRecord(record);
  ...
  // CRUD Actions
  const onSaveNew = () => {
    const newItem: CashiersStaffItem = {
  ...
  // Table Columns
  const columns: Column<CashiersStaffItem>[] = [
    { key: 'id', title: 'ID' },
    { key: 'name', title: 'Name' },
    { 
      key: 'status', 
      title: 'Status', 
      render: (record: CashiersStaffItem) => {
        let color = '#666';
        let bg = '#eee';
  ...
  return (
    <div className="dashboard-page">
      <PageHeader title="Cashiers & Staff" filterText="Last 30 Days" />
      ...
      <DataTable 
        title="Manage Cashiers & Staff" 
        description="View and manage all your cashiers & staff here."
        columns={columns} 
        data={data} 
