...
interface CouponsDiscountsItem {
  id: string;
  name: string;
  status: string;
  date: string;
...
export default function CouponsDiscountsPage() {
  ...
  // Data State
  const [data, setData] = useState<CouponsDiscountsItem[]>([
    { id: '1', name: 'Sample Coupons & Discounts A', status: 'Active', date: 'Oct 24, 2026' },
    { id: '2', name: 'Sample Coupons & Discounts B', status: 'Pending', date: 'Oct 25, 2026' },
    { id: '3', name: 'Sample Coupons & Discounts C', status: 'Inactive', date: 'Oct 26, 2026' },
  ]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<CouponsDiscountsItem | null>(null);
  ...
  const handleEditClick = (record: CouponsDiscountsItem) => {
    setSelectedRecord(record);
    setFormName(record.name);
    setIsEditModalOpen(true);
  };

  const handleViewClick = (record: CouponsDiscountsItem) => {
    setSelectedRecord(record);
    setIsViewModalOpen(true);
  };

  const handleDeleteClick = (record: CouponsDiscountsItem) => {
    setSelectedRecord(record);
  ...
  // CRUD Actions
  const onSaveNew = () => {
    const newItem: CouponsDiscountsItem = {
  ...
  // Table Columns
  const columns: Column<CouponsDiscountsItem>[] = [
    { key: 'id', title: 'ID' },
    { key: 'name', title: 'Name' },
    { 
      key: 'status', 
      title: 'Status', 
      render: (record: CouponsDiscountsItem) => {
        let color = '#666';
        let bg = '#eee';
  ...
  return (
    <div className="dashboard-page">
      <PageHeader title="Coupons & Discounts" filterText="Last 30 Days" />
      ...
      <DataTable 
        ...
        description="View and manage all your coupons & discounts here."
        columns={columns} 
        data={data} 
      ...
      <Modal 
        ...
        title="Add New Coupons & Discounts"
      >
