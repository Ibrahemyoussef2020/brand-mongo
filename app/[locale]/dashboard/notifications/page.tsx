...
export default function NotificationsPage() {
  ...
  const [data, setData] = useState<NotificationsItem[]>([
    { id: '1', name: 'Sample Notifications A', status: 'Active', date: 'Oct 24, 2026' },
    { id: '2', name: 'Sample Notifications B', status: 'Pending', date: 'Oct 25, 2026' },
    { id: '3', name: 'Sample Notifications C', status: 'Inactive', date: 'Oct 26, 2026' },
  ]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<NotificationsItem | null>(null);
  ...
  const handleEditClick = (record: NotificationsItem) => {
    setSelectedRecord(record);
    setFormData({ name: record.name, status: record.status });
    setIsEditModalOpen(true);
  };

  const handleViewClick = (record: NotificationsItem) => {
    setSelectedRecord(record);
    setIsViewModalOpen(true);
  };

  const handleDeleteClick = (record: NotificationsItem) => {
    setSelectedRecord(record);
  ...
  // CRUD Actions
  const onSaveNew = () => {
    const newItem: NotificationsItem = {
  ...
  // Table Columns
  const columns: Column<NotificationsItem>[] = [
    { key: 'id', title: 'ID' },
    { key: 'name', title: 'Name' },
    { 
      key: 'status', 
      title: 'Status', 
      render: (record: NotificationsItem) => {
        let color = '#666';
        let bg = '#eee';
  ...
  return (
    <div className="dashboard-page">
      <PageHeader title="Notifications" filterText="Last 30 Days" />
      ...
      <DataTable 
        ...
        description="View and manage all your notifications here."
        columns={columns} 
        data={data} 
