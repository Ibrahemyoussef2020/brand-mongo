...
export default function OffersPage() {
  ...
  // Data State
  const [data, setData] = useState<OffersItem[]>([
    { id: '1', name: 'Sample Offers A', status: 'Active', date: 'Oct 24, 2026' },
    { id: '2', name: 'Sample Offers B', status: 'Pending', date: 'Oct 25, 2026' },
    { id: '3', name: 'Sample Offers C', status: 'Inactive', date: 'Oct 26, 2026' },
  ]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<OffersItem | null>(null);
  ...
  const handleEditClick = (record: OffersItem) => {
    setSelectedRecord(record);
    setFormData({ name: record.name, status: record.status });
    setIsEditModalOpen(true);
  };

  const handleViewClick = (record: OffersItem) => {
    setSelectedRecord(record);
    setIsViewModalOpen(true);
  };

  const handleDeleteClick = (record: OffersItem) => {
    setSelectedRecord(record);
  ...
  // Table Columns
  const columns: Column<OffersItem>[] = [
    { key: 'id', title: 'ID' },
    { key: 'name', title: 'Name' },
    { 
      key: 'status', 
      title: 'Status', 
      render: (record: OffersItem) => {
        let color = '#666';
        let bg = '#eee';
  ...
  return (
    <div className="dashboard-page">
      <PageHeader title="Offers" filterText="Last 30 Days" />
      ...
      <DataTable 
        title="Manage Offers" 
        description="View and manage all your offers here."
        columns={columns} 
        data={data} 
