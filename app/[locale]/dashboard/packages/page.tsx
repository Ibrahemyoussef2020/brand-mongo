...
export default function PackagesPage() {
  ...
  // Data State
  const [data, setData] = useState<PackagesItem[]>([
    { id: '1', name: 'Sample Packages A', status: 'Active', date: 'Oct 24, 2026' },
    { id: '2', name: 'Sample Packages B', status: 'Pending', date: 'Oct 25, 2026' },
    { id: '3', name: 'Sample Packages C', status: 'Inactive', date: 'Oct 26, 2026' },
  ]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<PackagesItem | null>(null);
  ...
  const handleEditClick = (record: PackagesItem) => {
    setSelectedRecord(record);
    setFormData({ name: record.name, status: record.status });
    setIsEditModalOpen(true);
  };

  const handleViewClick = (record: PackagesItem) => {
    setSelectedRecord(record);
    setIsViewModalOpen(true);
  };

  const handleDeleteClick = (record: PackagesItem) => {
    setSelectedRecord(record);
  ...
  // Table Columns
  const columns: Column<PackagesItem>[] = [
    { key: 'id', title: 'ID' },
    { key: 'name', title: 'Name' },
    { 
      key: 'status', 
      title: 'Status', 
      render: (record: PackagesItem) => {
        let color = '#666';
        let bg = '#eee';
  ...
  return (
    <div className="dashboard-page">
      <PageHeader title="Packages" filterText="Last 30 Days" />
      ...
      <DataTable 
        ...
        description="View and manage all your packages here."
        columns={columns} 
        data={data} 
