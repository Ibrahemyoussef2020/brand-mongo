...
export default function POSDiscountsPage() {
  ...
  // Data State
  const [data, setData] = useState<POSDiscountsItem[]>([
    { id: '1', name: 'Sample POS Discounts A', status: 'Active', date: 'Oct 24, 2026' },
    { id: '2', name: 'Sample POS Discounts B', status: 'Pending', date: 'Oct 25, 2026' },
    { id: '3', name: 'Sample POS Discounts C', status: 'Inactive', date: 'Oct 26, 2026' },
  ]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<POSDiscountsItem | null>(null);
  ...
  const handleEditClick = (record: POSDiscountsItem) => {
    setSelectedRecord(record);
    setFormData({ name: record.name, status: record.status });
    setIsEditModalOpen(true);
  };

  const handleViewClick = (record: POSDiscountsItem) => {
    setSelectedRecord(record);
    setIsViewModalOpen(true);
  };

  const handleDeleteClick = (record: POSDiscountsItem) => {
    setSelectedRecord(record);
  ...
  // CRUD Actions
  const onSaveNew = () => {
    const newItem: POSDiscountsItem = {
  ...
  // Table Columns
  const columns: Column<POSDiscountsItem>[] = [
    { key: 'id', title: 'ID' },
    { key: 'name', title: 'Name' },
    { 
      key: 'status', 
      title: 'Status', 
      render: (record: POSDiscountsItem) => {
        let color = '#666';
        let bg = '#eee';
  ...
  return (
    <div className="dashboard-page">
      <PageHeader title="POS Discounts" filterText="Last 30 Days" />
      ...
      <DataTable 
        title="Manage POS Discounts" 
        description="View and manage all your pos discounts here."
        columns={columns} 
        data={data} 
