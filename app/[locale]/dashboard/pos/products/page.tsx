...
export default function POSProductsPage() {
  ...
  // Data State
  const [data, setData] = useState<POSProductsItem[]>([
    { id: '1', name: 'Sample POS Products A', status: 'Active', date: 'Oct 24, 2026' },
    { id: '2', name: 'Sample POS Products B', status: 'Pending', date: 'Oct 25, 2026' },
    { id: '3', name: 'Sample POS Products C', status: 'Inactive', date: 'Oct 26, 2026' },
  ]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<POSProductsItem | null>(null);
  ...
  const handleEditClick = (record: POSProductsItem) => {
    setSelectedRecord(record);
    setFormData({ name: record.name, status: record.status });
    setIsEditModalOpen(true);
  };

  const handleViewClick = (record: POSProductsItem) => {
    setSelectedRecord(record);
    setIsViewModalOpen(true);
  };

  const handleDeleteClick = (record: POSProductsItem) => {
    setSelectedRecord(record);
  ...
  // Table Columns
  const columns: Column<POSProductsItem>[] = [
    { key: 'id', title: 'ID' },
    { key: 'name', title: 'Name' },
    { 
      key: 'status', 
      title: 'Status', 
      render: (record: POSProductsItem) => {
        let color = '#666';
        let bg = '#eee';
  ...
  return (
    <div className="dashboard-page">
      <PageHeader title="POS Products" filterText="Last 30 Days" />
      ...
      <DataTable 
        title="Manage POS Products" 
        description="View and manage all your pos products here."
        columns={columns} 
        data={data} 
