...
export default function ReportsPage() {
  ...
  // Data State
  const [data, setData] = useState<ReportsItem[]>([
    { id: '1', name: 'Sample Reports A', status: 'Active', date: 'Oct 24, 2026' },
    { id: '2', name: 'Sample Reports B', status: 'Pending', date: 'Oct 25, 2026' },
    { id: '3', name: 'Sample Reports C', status: 'Inactive', date: 'Oct 26, 2026' },
  ]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<ReportsItem | null>(null);
  ...
  const handleEditClick = (record: ReportsItem) => {
    setSelectedRecord(record);
    setFormData({ name: record.name, status: record.status });
    setIsEditModalOpen(true);
  };

  const handleViewClick = (record: ReportsItem) => {
    setSelectedRecord(record);
    setIsViewModalOpen(true);
  };

  const handleDeleteClick = (record: ReportsItem) => {
    setSelectedRecord(record);
  ...
  // Table Columns
  const columns: Column<ReportsItem>[] = [
    { key: 'id', title: 'ID' },
    { key: 'name', title: 'Name' },
    { 
      key: 'status', 
      title: 'Status', 
      render: (record: ReportsItem) => {
        let color = '#666';
        let bg = '#eee';
  ...
  return (
    <div className="dashboard-page">
      <PageHeader title="Reports" filterText="Last 30 Days" />
      ...
      <DataTable 
        ...
        description="View and manage all your reports here."
        columns={columns} 
        data={data} 
