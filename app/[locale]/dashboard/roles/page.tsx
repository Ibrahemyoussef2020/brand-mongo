...
export default function RolesPermissionsPage() {
  ...
  // Data State
  const [data, setData] = useState<RolesPermissionsItem[]>([
    { id: '1', name: 'Sample Roles & Permissions A', status: 'Active', date: 'Oct 24, 2026' },
    { id: '2', name: 'Sample Roles & Permissions B', status: 'Pending', date: 'Oct 25, 2026' },
    { id: '3', name: 'Sample Roles & Permissions C', status: 'Inactive', date: 'Oct 26, 2026' },
  ]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<RolesPermissionsItem | null>(null);
  ...
  const handleEditClick = (record: RolesPermissionsItem) => {
    setSelectedRecord(record);
    setFormData({ name: record.name, status: record.status });
    setIsEditModalOpen(true);
  };

  const handleViewClick = (record: RolesPermissionsItem) => {
    setSelectedRecord(record);
    setIsViewModalOpen(true);
  };

  const handleDeleteClick = (record: RolesPermissionsItem) => {
    setSelectedRecord(record);
  ...
  // CRUD Actions
  const onSaveNew = () => {
    const newItem: RolesPermissionsItem = {
  ...
  // Table Columns
  const columns: Column<RolesPermissionsItem>[] = [
    { key: 'id', title: 'ID' },
    { key: 'name', title: 'Name' },
    { 
      key: 'status', 
      title: 'Status', 
      render: (record: RolesPermissionsItem) => {
        let color = '#666';
        let bg = '#eee';
  ...
  return (
    <div className="dashboard-page">
      <PageHeader title="Roles & Permissions" filterText="Last 30 Days" />
      ...
      <DataTable 
        ...
        description="View and manage all your roles & permissions here."
        columns={columns} 
        data={data} 
